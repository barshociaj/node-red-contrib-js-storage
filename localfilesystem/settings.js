/**
 * Node-RED source: https://github.com/node-red/node-red/blob/master/packages/node_modules/%40node-red/runtime/lib/storage/localfilesystem/settings.js
...
async function migrateToMultipleConfigFiles() {
  ...
  const data = await util.readFile(
    globalSettingsFile,
...
async function readSettings() {
  ...
  const result = await util.readFile(
    runtimeFilename,
    runtimeFilename + ".backup",
    {}
  );
  ...
  configSections.forEach((key) => {
    ...
    readPromises.push(
      util
        .readFile(sectionFilename, sectionFilename + ".backup", {})
        .then((sectionData) => {
